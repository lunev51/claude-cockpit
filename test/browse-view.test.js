...
test('лишние ведущие слеши не выбрасывают путь из UNC-разбора', async () => {
  // M-4 финального ре-ревью: разбор требовал РОВНО двух ведущих разделителей,
  // и '\\\\\\\\srv' (опечатка, двойная вставка, '////srv' из строки ввода) уходил
  // в обычную ветку — крошка 'srv\\' резолвилась относительно рабочей папки
  // главного процесса кокпита. Это дословно тот же дефект, что чинили дважды.
  const { crumbs } = await import(url);
  assert.deepStrictEqual(crumbs('\\\\\\\\srv\\share'), [
    { name: '\\\\srv\\share', path: '\\\\srv\\share' },
  ]);
  assert.deepStrictEqual(crumbs('\\\\\\srv'), [
    { name: '\\\\srv', path: '\\\\srv' },
