...
test('groupOf: незнакомый статус не роняет раскладку', async () => {
  // Статусы ядра могут прирасти раньше, чем сайдбар про них узнает.
  const { groupOf } = await load();
  assert.strictEqual(groupOf('какой-то-новый'), 'ready');
});
