...
test('не отвечающая буква диска не задерживает список', async () => {
  ...
  const drives = await listDrives({
    ...
    // Своё состояние ОБЯЗАТЕЛЬНО: без него вызов попадает в общий кэш
    // driveState, куда соседний тест («диски: непустой список») уже сложил
    // НАСТОЯЩИЙ состав дисков этой машины. Подменённая проба тогда не
    // вызывается вовсе, и тест проверяет кэш вместо логики. Пока на машине был
    // один диск C:, ответ случайно совпадал с ожидаемым и тест выглядел
    // зелёным; появление второго диска (G:) это вскрыло — 10.08.
    drives: { hung: new Set() },
  });
  const elapsed = Date.now() - started;
...
test('таймаут буквы отсчитывается от старта пробы, а не от постановки в очередь', async () => {
  ...
  const drives = await listDrives({
    timeoutMs: 60,
    concurrency: 1,
    // Своё состояние — по той же причине, что в тесте выше: иначе ответ
    // приедет из общего кэша с реальными дисками машины.
    drives: { hung: new Set() },
    probe: (root) => {
