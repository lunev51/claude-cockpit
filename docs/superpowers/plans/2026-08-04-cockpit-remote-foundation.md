...
- [ ] **Шаг 5: Подключить выбор транспорта**

**ИСПРАВЛЕНО ПОСЛЕ РЕВЬЮ (Critical 1).** Первая версия этого шага вставляла
`<script type="module" src="./js/api-boot.js">` в разметку **перед** `app.js`, полагая,
что ожидание внутри него задержит запуск приложения. Это неверно: ожидание верхнего
уровня в модуле задерживает только те модули, которые его **импортируют**, а соседний
независимый `<script type="module">` стартует сразу. Проверено в живом Chromium:
`app.js` вызывал `boot()`, тот первой строкой лез в `window.api`, получал `undefined`
и падал с `Cannot read properties of undefined (reading 'config')` — страница
оставалась пустой навсегда. Смоук этого не ловил, потому что в Electron ветка
`if (!window.api)` ложна.

Правильный способ — сделать `app.js` импортирующим. В `src/renderer/js/app.js` первым
импортом добавить:

```js
// Выбор транспорта. В Electron window.api уже собран preload'ом; в браузере
// его собирает сетевой мост поверх WebSocket. Это ИМПОРТ, а не отдельный
// тег в разметке: только импорт заставляет app.js дождаться, пока мост
// поднимет сокет (см. разбор Critical 1 выше).
import './api-boot.js';
```

Разметку `index.html` при этом не трогать вовсе.
...
**ИСПРАВЛЕНО ПОСЛЕ РЕВЬЮ (Critical 2).** Одиннадцать методов `preload.js` не передают
аргументы списком, а **переупаковывают их в один объект**: `write: (tabId, data) =>
ipcRenderer.send('term:write', { tabId, data })`. Мост, шлющий сырой список, дал бы
обработчику строку вместо объекта, а там стоит `if (!payload || typeof payload !==
'object') return` — команда пропала бы молча, без ошибки клиенту. Затронуты
`tabs:seen`, `term:start`, `term:write`, `term:resize`, `term:restart`,
`workspace:setActive`, `ghost:save`, `attention:update`, `queue:add`, `queue:remove`,
`queue:clear`: то есть ввод с клавиатуры, запуск и ресайз терминала, очередь промптов,
активная вкладка и черновики.

Поэтому у таких методов в форме есть поле `pack` — имена полей объекта в том порядке,
в каком метод принимает аргументы. Имена брать из `preload.js` ДОСЛОВНО. Мост при
наличии `pack` собирает объект и шлёт `args: [объект]`; без `pack` — как есть.
Упаковка живёт на стороне браузера, сервер её не знает и остаётся простым.
...
// отсутствует и падает в неожиданный момент, а не при загрузке. Сверять надо
// не только состав каналов, но и вид вызова, имя метода и состав pack:
// смена вида или переименование ломают браузер так же молча.
