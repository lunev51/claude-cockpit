...
const DEFAULTS = {
  ...
  bridge: { port: 48200 },
  // Задача 7 фазы «кокпит по сети»: адрес и порт сетевого сервера (net-server.js).
  // Дефолт '127.0.0.1' — осознанно: аутентификации в этой фазе ещё нет, поэтому
  // без явной настройки сервер виден только с этого же ПК. Реальный адрес
  // Tailscale прописывается пользователем в config.user.json (userData),
  // НЕ здесь — он принадлежит машине, а не программе, и меняется при
  // перевыпуске ключа Tailscale.
  //
  // allowedHosts (соседняя задача, фикс DNS-rebinding в net-server.js,
  // раунд 4): доп. имена для проверки заголовка Host — к машине приходят и
  // по адресу, и по DNS-имени тайлнета (cockpit-desktop.tailXXXX.ts.net)
  // одновременно, а host выше хранит обычно только одно из двух. Пустой
  // массив по умолчанию — net-server.js и без него собирает белый список из
  // host/127.0.0.1/localhost, allowedHosts только дополняет его.
  net: { host: '127.0.0.1', port: 48300, allowedHosts: [] },
