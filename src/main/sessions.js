...
function createSessionManager({
  ...
  // это закрывает весь класс гонок (включая гипотетическую опоздавшую
  // SessionStart, которая иначе могла бы перепривязать sessionId вкладки на
  // чужую сессию), а не только тот единственный сценарий, где нашёлся живой
  // побочный эффект.
  function applyHookEvent(tabId, event, data = {}, gen = null) {
    const tab = tabs.get(tabId);
    if (!tab) return;
    if (gen !== null && gen !== undefined && gen !== tab.gen) return; // хвост чужого поколения — не наше
