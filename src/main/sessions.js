...
function createSessionManager({
  ...
  function start(tabId, cols, rows) {
    ...
    // Провал этого резюма не создаёт петли: onExit увидит usedOverride без
    ...
    if (tab.sessionId && !tab.pendingExtraArgs && !tab.resumeOnFirstSpawn) {
      ...
      // Обещание строкой выше выполняется ТОЛЬКО с этим сбросом (ревью 09.08,
      // B6). onExit считает провал как `usedOverride && !tab.sessionBound`, а
      // у вкладки, которая уже жила и была привязана, sessionBound остаётся
      // true с прошлой жизни. Без сброса провал резюма не детектился вовсе:
      // не обнулялся протухший sessionId, не срабатывало авто-восстановление,
      // мёртвый id уезжал в манифест — и следующий запуск кокпита снова
      // резюмил мертвеца. Это был фейл-луп, который в трёх других местах
      // файла закрыт, а здесь оставался открытым.
      //
      // restart() делает ровно то же (см. FIX 3 ниже) — теперь оба пути к
      // «подними процесс заново» ведут себя одинаково. Новый SessionStart
      // перевыставит флаг, когда сессия действительно привяжется.
      tab.sessionBound = false;
      // autoRecovered здесь трогать НЕ нужно, хотя restart() это делает.
      // Держит инвариант «sessionId ≠ null ⇒ autoRecovered === false»: флаг
      // взводится единственным местом — веткой провала оверрайда в onExit, а
      // она в том же блоке обнуляет sessionId; обратно ненулевым sessionId
      // делают только open() (флаг там инициализирован false) и bindSession
      // (гасит его тем же движением). Попасть сюда можно лишь при живом
      // sessionId, значит флаг уже false. Симметричная строка выглядела бы
      // уместной, но не держала бы ничего — мутация показала, что её удаление
  ...
  function list() {
    ...
    return [...tabs.values()].map(({
      ...
      waitingKind, waitingText, gen, sessionLabel, queue,
    }) => (
      {
        tabId, cwd, name, alive, status, subtitle, sessionId, ghostId,
        waitingKind, waitingText, gen, sessionLabel,
        // queue (ревью 09.08, B9) — тот же пробел, что уже закрывали для
        // waitingText, только про очередь промптов. Клиент собирает её
        // ИСКЛЮЧИТЕЛЬНО из событий queue:changed, а подключившийся к живым
        // вкладкам (перезагрузка страницы, браузер с макбука, второе окно)
        // ни одного такого события не застал — и видел пустую строку чипов
        // там, где в вкладке реально ждут промпты. Человек добавлял их
        // повторно, и на следующем Stop main вбрасывал всё дважды.
        // Копия, а не сам массив: снаружи он изменяемый.
        queue: queue.slice(),
      }
    ));
  }
