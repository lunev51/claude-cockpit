...
function createSessionManager({
  ...
  function spawn(tab) {
    ...
    // Прошлая смерть вкладки перестаёт быть фактом о ней ЗДЕСЬ, до вызова
    // фабрики. Гард ниже (`tab.status !== 'dead'`) задумывался против
    // СИНХРОННОГО exit из самой фабрики — «процесс умер, не успев родиться,
    // не воскрешаем». Но статус 'dead' живёт на вкладке и после обычной
    // смерти процесса, а restart() его не сбрасывает — и рестарт УЖЕ мёртвой
    // вкладки (тот самый Ctrl+Shift+R, который кокпит сам печатает строкой
    // «процесс завершён (код N) — Ctrl+Shift+R для перезапуска») спавнил pty
    // и тут же от него отрекался: tab.proc оставался null, term:started не
    // уходил, ввод был мёртв навсегда, а сам процесс — осиротевшим (close()
    // и следующий restart() его уже не убьют, каждый Ctrl+Shift+R добавлял
    // бы ещё один). Найдено живой проверкой фикс-волны финального ревью:
    // вкладка после /exit печатала в терминал из процесса, которым менеджер
    // не владел. Сброс здесь возвращает гарду ровно тот смысл, ради которого
    // он писался: 'dead' ПОСЛЕ вызова фабрики может означать только смерть
    // внутри неё. Событие не эмитим — статус всё равно выставится ниже
    // ('ready' на успехе, 'dead' в catch), лишний tab:status с пустым
    // статусом только сбил бы renderer.
    if (tab.status === 'dead') tab.status = null;
  ...
  function start(tabId, cols, rows) {
    ...
    tab.cols = cols;
    tab.rows = rows;
    // Important 2 (ре-ревью фикс-волны): у вкладки есть ПРИВЯЗАННАЯ сессия, а
    // процесса нет — значит этот спавн обязан её ПРОДОЛЖИТЬ, а не завести
    // новую. Без этой строки start() уходил на голые args: флаг
    // resumeOnFirstSpawn одноразовый и давно израсходован первым спавном
    // вкладки, так что --resume не достраивался ниоткуда — поднималась чистая
    // сессия, а прежняя оставалась только в транскрипте, никому не видимая.
    // restart() в том же положении резюмит (см. ниже), то есть ЯДРО вело себя
    // по-разному на двух путях к одному и тому же «подними процесс заново».
    //
    // Гард от потери данных обязан жить здесь, а не у вызывающего: renderer
    // сейчас не зовёт start() на мёртвой вкладке (shouldStartPty,
    // attach-guards.js), но это его сегодняшнее поведение — статика отдаётся
    // без заголовков кеширования, и «браузер держит старый скрипт» не теория.
    // Ядро не должно полагаться на дисциплину клиента там, где ценой ошибки
    // является потерянная сессия.
    //
    // Условия узкие, чтобы не отобрать решение у тех, кто уже его принял:
    //  - pendingExtraArgs занят → restart() уже выбрал, чем спавнить (в т.ч.
    //    сознательно голыми args после провала резюма — не зацикливаемся);
    //  - resumeOnFirstSpawn → восстановленная вкладка, spawn() сам достроит
    //    --resume и сам погасит флаг; дублировать нельзя.
    // Провал этого резюма не создаёт петли: онExit увидит usedOverride без
    // sessionBound, обнулит sessionId и уйдёт в авто-восстановление — ровно
    // тот же путь, что уже защищает restart().
    if (tab.sessionId && !tab.pendingExtraArgs && !tab.resumeOnFirstSpawn) {
      tab.pendingExtraArgs = ['--resume', tab.sessionId];
    }
    spawn(tab);
  }
