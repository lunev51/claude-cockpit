...
const DEFAULT_CONFIG = {
  ...
  maxResets: 4,
  retryMs: 300000,
  // Ревью 09.08: было 3 попытки по 5 минут — окно ~16 минут, после которого
  // ночная смена сдаётся до утра. Теперь интервал удваивается (5 → 10 → 20 →
  // 30 → 30 мин при потолке), и окно вырастает примерно до полутора часов —
  // этого хватает пережить ночной отвал сети или бэкофф авторизации.
  maxRetries: 5,
  retryMaxMs: 1800000,
  // Как часто сторож сверяет «не вышел ли срок» — см. watchdogTick.
  watchdogMs: 60000,
};
...
function createNightWatch({
  ...
  // Сторож будильника (ревью 09.08, находка A7). Вся фича держалась на ОДНОМ
  // длинном таймере до момента сброса лимита. powerSaveBlocker удерживает
  // систему от АВТОсна, но не от ручного «Сон», закрытой крышки, гибернации по
  // разряду и собственного отказа; а поведение длинных таймеров после resume
  // на Windows недетерминировано — таймер может «доспать» остаток уже после
  // пробуждения машины или не выстрелить до утра. Отказ при этом тихий: в
  // журнале остаются armed + limit-stop и больше ничего, хотя фича существует
  // ровно ради этого момента.
  //
  // Сторож — короткий повторяющийся таймер: он лишь сверяет now() с wakeAt и,
  // если срок вышел, будит сам. Короткие таймеры переживают resume несравнимо
  // надёжнее длинных, а цена — один сравнение раз в минуту.
  ...
  // Тик сторожа: срок вышел — будим сами, нет — ждём дальше. Сам по себе он
  // ничего не решает и никуда не ходит, вся логика пробуждения остаётся в
  // doWake (она идемпотентна и умеет разбираться с опустевшим pending).
  function watchdogTick() {
    watchdogTimer = null;
    if (!armed || wakeAt === null) return; // ждать больше нечего
    if (now() >= wakeAt) {
      doWake();
      return;
    }
  ...
  function armWatchdog() {
    stopWatchdog();
    if (!armed || wakeAt === null) return;
    watchdogTimer = setTimer(watchdogTick, cfg.watchdogMs);
  }
  ...
  function stopWaitTimer() {
    ...
    wakeAt = null;
    stopWatchdog(); // сторожить больше нечего
  }
  ...
  // Рядом взводится сторож: он подстрахует, если длинный таймер не переживёт
  // сон машины.
  function scheduleWaitAt(atMs) {
    stopWaitTimer();
    wakeAt = atMs;
    // Сторож планируется ПЕРВЫМ, будильник — вторым, и это не косметика:
    // тесты ядра опознают «последний запланированный таймер» как будильник и
    // ассертят его задержку (перепутать staggerMs с retryMs здесь легко и
    // незаметно — см. комментарий у фейка таймеров). Обратный порядок сделал
    // бы «последним» сторожа и обесценил бы полсотни таких проверок.
    armWatchdog();
  ...
  async function doWake() {
    // Сюда ведут ДВА пути: штатный будильник и сторож (см. watchdogTick), и
    // после сна машины оба могут выстрелить почти одновременно — сторож
    // заметил просрочку, а следом Node всё же отработал протухший таймер.
    //
    // Первый вошедший снимает таймер конкурента — именно stopWaitTimer(), а
    // не `waitTimer = null`. Разница решает судьбу ночи (ревью фикса A7):
    // когда будит СТОРОЖ, длинный будильник ещё жив, и обнуление хендла без
    // clearTimer просто теряло его. Он выстреливал сиротой часы спустя — уже
    // посреди СЛЕДУЮЩЕГО цикла ожидания: сбрасывал его wakeAt, снимал его
    // сторожа, видел «окно не сброшено» и уводил ночь в ретраи до gave-up,
    // после которого pending чистится, блокер снимается и машина засыпает.
    // То есть фикс, написанный против тихого отказа, сам порождал такой же.
    //
    // Побочно это закрывает и двойной проход: в ретрай-ветке pending не
    // очищается, и два прохода нарастили бы retries вдвое, спалив окно.
    // Отдельный флаг «проход уже идёт» после этого не нужен — конкурентный
    // вход становится невозможен структурно.
    //
    // В штатном пути (будильник выстрелил сам) clearTimer на уже
    // отработавшем id — no-op, вреда нет.
    stopWaitTimer();
    ...
    try {
      ...
      if (windowReset) {
        ...
        if (retries <= cfg.maxRetries) {
          // Бэкофф с удвоением и потолком (ревью 09.08). Раньше это были три
          // попытки с одним интервалом — всего около шестнадцати минут, после
          // чего pending чистился, блокер снимался и машина засыпала до утра.
          // Любой ночной транзиент длиннее съедал всю ночь целиком: отвал
          // Wi-Fi на USB-адаптере, 401 до обновления токена (у него свой
          // пятнадцатиминутный бэкофф), 429 с десятиминутным. Цена ожидания —
          // один HTTP-запрос при уже удерживаемом блокере, то есть ноль.
          const delay = Math.min(cfg.retryMs * (2 ** (retries - 1)), cfg.retryMaxMs);
          appendJournal({ type: 'retry', detail: `${retries} (${tabIds})` });
          scheduleWaitAt(now() + delay);
          emitChange();
        } else {
...
// DEFAULT_CONFIG экспортируется ради сверки с дефолтами config.js: значения
// оттуда затирают здешние, и разошедшийся ключ означает, что правка ядра не
// доехала до боя (см. test/night-watch-config.test.js).
module.exports = { createNightWatch, DEFAULT_CONFIG };
