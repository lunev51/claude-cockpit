    ...
    return new Promise((resolve) => {
      ...
      fs.stat(filePath, (statErr, st) => {
        ...
        fs.open(filePath, 'r', async (openErr, fd) => {
          ...
          try {
            const { size } = st;
            // Файл целиком укладывается в бюджет двух кусков — читаем его
            // ОДНИМ куском в prefix, а suffix оставляем пустым. Это не
            // экономия, а корректность: read() ищет custom-title в хвосте, и
            // только при ПУСТОМ хвосте просматривает на него весь префикс.
            //
            // Два прошлых раунда ревью споткнулись здесь по очереди. Сперва
            // хвост у файлов чуть длиннее префикса не читался вовсе, и байты
            // между 256 КБ и концом файла не читал никто. Потом читать их
            // стали — но всё равно не смотрели на `/rename`: непустой хвост
            // (пусть даже в один байт) отменял просмотр префикса, и файл
            // размером 262 145 байт терял имя, которое файл размером
            // 262 144 байта находил. Полоса отказа получалась (256 КБ,
            // 256 КБ + d], где d — расстояние от последней копии записи до
            // конца файла, а оно по замерам доходит до 88 КБ.
            //
            // Пока куски НЕПРЕРЫВНЫ (size <= prefixBytes + suffixBytes),
            // prefix и есть весь файл, и просмотр по нему не может вернуть
            // протухшее имя: берётся последнее вхождение во всём файле.
            if (size <= prefixBytes + suffixBytes) {
              resolve({ prefix: await readChunk(fd, size, 0), suffix: '' });
              return;
            }
            // Файл длиннее бюджета: между кусками остаётся непрочитанный
            // разрыв. Это осознанный предел — custom-title переиздаётся на
            // чекпоинтах с интервалом до 89 640 байт (замер), так что
            // последняя копия почти всегда в пределах 256-килобайтного
            // хвоста. Полное чтение 226-МБ транскрипта не рассматривается:
            // это ровно тот дисковый шторм, что уже вешал машину.
            const prefix = await readChunk(fd, prefixBytes, 0);
            const suffix = await readChunk(fd, suffixBytes, size - suffixBytes);
