...
// Видно ли сейчас интерфейс. Источник зависит от того, кто мы (ревью 09.08, B7):
//
//  - окно ПК спрашивает main. document.hidden здесь ненадёжен: окно создано с
//    backgroundThrottling:false, и по документации Electron его состояние
//    остаётся visible даже когда окно скрыто в трей, а автозапуск с --hidden
//    (show:false) даёт visible у окна, которого человек ни разу не видел;
//  - браузер пользуется своим document.hidden: событие про окно ПК ему не про
//    него, он живёт на другой машине.
//
// Начальное значение — true: до ответа main лучше лишний раз перерисовать,
// чем молчать в видимом окне.
let pcWindowVisible = true;
const isNetClient = () => !!window.__cockpitNetClient;
function uiHidden() {
  return isNetClient() ? document.hidden : !pcWindowVisible;
}
...
async function attachTab(t, { activate = false } = {}) {
  ...
  tabStore.add(t);
  // Очередь промптов из менеджера (ревью 09.08, B9). Клиент собирает её из
  // событий queue:changed, а подключившийся к УЖЕ живым вкладкам ни одного
  // такого события не застал: строка чипов показывала пустоту там, где во
  // вкладке реально ждут промпты, человек добавлял их повторно, и на
  // следующем Stop main вбрасывал всё дважды. Тот же приём, что со статусом
  // ниже: берём состояние из списка, а не ждём следующего события.
  //
  // `!queueByTab.has` обязателен (замечание ревью фикса): снапшот t.queue
  // посчитан ОДНИМ tabs:list на весь пакет вкладок, а подключаемся мы по
  // очереди, ожидая историю вывода каждой. За это время по вкладке мог прийти
  // queue:changed — и он свежее снапшота. Запись существует только если
  // событие уже случилось (подписка ставится раньше bootWorkspace), поэтому
  // «есть запись» и означает «наше зеркало новее».
  if (Array.isArray(t.queue) && t.queue.length && !queueByTab.has(t.tabId)) {
    queueByTab.set(t.tabId, t.queue.slice());
    renderQueueBar();
  }
...
// «Пока мы сверялись, состав успел измениться ещё раз». Ревью 09.08 (B5):
// одноразовость гасила такие события НАСОВСЕМ. Снимок берётся в начале, а сам
// проход долгий — attachTab на каждую новую вкладку ждёт историю вывода
// (net:buffer). Всё, что сосед сделал внутри этого окна, попадало в «уже
// идёт» и терялось до следующего tabs:changed, которого может не быть часами.
// Это ровно тот класс, что дважды давал живые жалобы («открыл на маке — на ПК
// нет», «сессии задвоились»).
let tabsSyncDirty = false;
async function syncTabsFromMain() {
  ...
  // ходит в main и заводит xterm'ы. Событие, пришедшее во время прохода, не
  // запускает второй параллельно, а помечает состав грязным — пересверимся
  // сразу после текущего прохода.
  if (tabsSyncRun) {
    tabsSyncDirty = true;
    return tabsSyncRun;
  }
  ...
  try {
    ...
    // Замечание ревью фикса: tabs.list уже под своим try, но фаза применения
    // (detachTabLocal/attachLiveTabs) — нет. Без этого catch исключение оттуда
    // перепрыгивало бы пересверку ниже, оставляя состав помеченным грязным до
    // следующего события, а сам вызов давал unhandled rejection: подписчик
    // tabs:changed зовёт нас fire-and-forget.
    console.warn('[вкладки] применение сверки не удалось:', err && err.message);
  } finally {
    tabsSyncRun = null;
  }
  // Пересверка ровно одна, сколько бы событий ни пришло за время прохода:
  // флаг гасим ДО повторного вызова, а тот при новых событиях взведёт его
  // заново. Ошибка прохода не повод бросать грязный флаг — иначе одно
  // неудачное обращение к main похоронило бы и следующую сверку.
  if (tabsSyncDirty) {
    tabsSyncDirty = false;
    await syncTabsFromMain();
  }
  return undefined;
}
...
async function boot() {
  ...
  // если он открыт).
  //
  // В скрытом окне (спрятано в трей) обратный отсчёт перерисовывать некому —
  // пропускаем тик, а при возвращении досчитываем разом (см. слушатель
  // visibilitychange ниже). Ревью 09.08, B7.
  setInterval(() => {
    if (uiHidden()) return;
    redrawUsageViews();
  }, 30000);
  ...
  //
  // Ревью 09.08 (B7): в СКРЫТОМ окне этот веер бессмыслен вдвойне. TTL кэша
  // (150 с) намеренно меньше периода (180 с), то есть каждый тик — реальные
  // спавны gh по числу уникальных папок, и каждый под shell:true, то есть на
  // Windows это ещё и cmd.exe-обёртка. Окно, спрятанное в трей, делало это
  // часами, а бейджи в это время никто не видит. Кольца лимитов там же
  // перерисовывались каждые 30 с в никуда.
  //
  // Гасим оба на скрытом окне и догоняем разом, когда человек вернулся: так
  // данные к моменту показа свежие, а фон не работает вхолостую.
  setInterval(() => {
    if (uiHidden()) return;
    for (const tabId of tabStore.order()) refreshTabPr(tabId);
  }, 180000);

  // Догон при возвращении: показываем не то, что было час назад. Кольца берут
  // готовый снимок из main (сети не касаются), бейджи — из кэша gh-info, где
  // спавн случится только при реально протухшем TTL, а одинаковые папки
  // схлопывает single-flight.
  const догнать = () => {
    if (uiHidden()) return;
    redrawUsageViews();
    for (const tabId of tabStore.order()) refreshTabPr(tabId);
  };
  // Два источника, по одному на каждого клиента (см. uiHidden выше).
  document.addEventListener('visibilitychange', догнать);
  window.api.window?.onVisibility?.(({ visible }) => {
    // Событие про окно ПК. Браузеру оно не про него: там своя видимость, и
    // подмена ею сломала бы гейт на чужом состоянии.
    if (isNetClient()) return;
    pcWindowVisible = !!visible;
    догнать();
  });
  // Начальное состояние окна ПК: автозапуск поднимает кокпит сразу в трей, и
  // без этого запроса мы считали бы его видимым до первого показа.
  if (!isNetClient()) {
    window.api.window?.isVisible?.()
      .then((visible) => { pcWindowVisible = visible !== false; })
      .catch(() => { /* старый main без канала — остаёмся при «видимо» */ });
  }
