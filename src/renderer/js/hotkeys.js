...
const GROUPS = [
  {
    ...
    keys: [
      ...
      ['Ctrl+O', 'Файловый обзор — новая сессия из любой папки'],
  ...
  },
  {
    title: 'Терминал',
    keys: [
      ['Ctrl+Shift+R', 'Перезапустить сессию (то же — кнопка ⟳ в панели снизу)'],
      ['Ctrl+Shift+F', 'Поиск по буферу текущего терминала'],
      ['Ctrl+Shift+C / Ctrl+Shift+V', 'Копировать / вставить (в т.ч. скриншот)'],
      // Без этой строки фичу не найти: Claude Code перехватывает мышь, и
      // обычная протяжка НИЧЕГО не выделяет — человек видит только то, что
      // копирование «не работает» (живая жалоба 11.08). Клавиша разная по
      // платформам, потому что так устроен xterm: на маке форсирует выделение
      // Option, на Windows — Shift.
      ['Option (мак) / Shift (ПК) + протяжка', 'Выделить текст, пока сессия занимает мышь'],
